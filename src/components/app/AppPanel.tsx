 ...
 * stay whole. It shows the two ticked rows at every width: 36px rows keep it inside the 4:3 frame
 * at 390, and from 1024, where the frame shares its 4:3 box with the card's caption, it sits 24px
 * from the top with about 36px of ground under it at 1440, and at 1280 the crop takes only its foot.
 */
function Fragment() {
  return (
    <div className="rounded-hard border border-hairline bg-raised py-4 pl-4 pr-10 text-ink">
      ...
      <ul className="mt-3 border-t border-hairline">
        {appPanel.rows.slice(0, 2).map((name, i) => (
          <Row key={name} name={name} text={rowText("select", i)} className="flex h-9" />
        ))}
      </ul>
