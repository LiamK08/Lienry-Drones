...
- Radius: one token, 4px, on every button, card, input, image and container (`rounded-hard`); there is no full radius token, so nothing can become a pill. No chips or badges, and nothing sits above a heading: supporting text is plain sentence-case sans, and the one uppercase style is the 12px label described above. No tinted backgrounds, glows, blurred blobs, text shadows, gradient text or drop shadows; hairline borders and flat section colours only.
