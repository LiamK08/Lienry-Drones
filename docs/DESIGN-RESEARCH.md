...
| Typeface | One variable grotesk (Aktiv Grotesk VF), weight 400 everywhere, 300 for big numerals | A single weight forces hierarchy through size and space, which reads as confident | Two self-hosted Fontsource families: Instrument Serif at weight 400 for headings, Inter with optical sizing for everything else. Different families, and hierarchy still comes from size and space rather than weight |
| Desktop scale | H1 56px / 59px line-height / -2.25px tracking; H3 41px; H4 33.7px; body 15px / 21px; small 13.2px / 18.5px; caption 12px / 16.8px | Tight tracking at display sizes, generous line-height at text sizes; the headline is one line at 1440 | Lienry at 1440: display 96px, H1 64px, H2 44px, H3 28px, lead 20px, body 18px / 1.55, small 14px, label 12px at 0.12em. Headings run -0.02em at 1.05 to 1.15 |
| Mobile scale | H1 41px; H3 27.4px; H4 24px; body 14px / 19.6px | Mobile headlines drop by roughly 0.7x, body barely changes | Lienry at 390: display 64px, H1 44px, H2 32px, H3 24px, body 17px. One clamp() per step carries each one between the two ends |
...
| Buttons | 48px-radius pills, 30px tall, 12px labels; chips at 24px radius | Small pills feel precise; the pill shape is Legora's signature | Lienry uses 4px-radius rectangles, the one radius token on the site, at 30px in the header with a 13px label, 44px on pages and 48px in the hero, both with a 14px label. A rectangle, never a pill |
| Containers | Max widths 680px (text), 860px (mixed), 1080px (wide); gutters 24px | Narrow text measure keeps paragraphs at 60 to 70 characters | 68-character text measure, 880px statements, 1280px wide, 24px gutters, 12-column grid |
...
Lienry's take: design the mobile layouts first for the hero, explainer and carousel. Layer toggles become a row of 4px-radius rectangles above the image, the 3D building is replaced by a short video on phones, and the footer uses the same accordion pattern.
